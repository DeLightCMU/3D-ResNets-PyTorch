...
def convert_csv_to_dict(labels, csv_path, subset):
    data = pd.read_csv(csv_path, delimiter=' ', header=None)
    keys = []
    key_labels = []
    for i in range(data.shape[0]):
        class_name = labels[data.ix[i, 1]]
        basename = data.ix[i, 0]
...
def convert_ucf101_csv_to_activitynet_json(label_csv_path, train_csv_path,
    ...
    train_database = convert_csv_to_dict(labels, train_csv_path, 'training')
    val_database = convert_csv_to_dict(labels, val_csv_path, 'validation')
...
if __name__ == '__main__':
    ...
    dst_json_path = os.path.join(csv_dir_path, 'kinetics.json')
